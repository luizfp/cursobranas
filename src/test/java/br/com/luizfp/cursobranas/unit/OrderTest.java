...
public final class OrderTest {
    ...
    void shouldCreateAnOrder() {
        assertThat(new Order(CPF, OffsetDateTime.now())).isNotNull();
    }

    @Test
    void shouldThrowsErrorWithInvalidCpf() {
        Assertions.assertThrows(InvalidCpfException.class, () -> new Order("111.111.111-11", OffsetDateTime.now()));
    }

    @Test
    void shouldCreatOrderWithRightTotalItems() {
        final Order order = new Order(CPF, OffsetDateTime.now());
